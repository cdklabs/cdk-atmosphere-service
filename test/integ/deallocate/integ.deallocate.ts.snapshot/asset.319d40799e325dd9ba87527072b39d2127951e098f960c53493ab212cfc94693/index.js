...
async function handler3(event) {
  ...
  try {
    ...
    const pool = (await clients3.allocations.get(allocationId)).pool;
    const log = new Logger({ allocationId, pool, component: "deallocate" });
    return await doHandler3(allocationId, request, log);
  } catch (e) {
    return failure2(e instanceof ProxyError2 ? e.statusCode : 500, e.message);
  }
}
...
async function handler4(event) {
  ...
  try {
    return await doHandler4(event, log);
  } catch (e) {
    log.error(e);
    throw e;
  }
