...
var CDK_ATMOSPHERE_INTEG_ENV = "CDK_ATMOSPHERE_INTEG";
...
var ecs = new import_client_ecs3.ECS();
...
var Session = class _Session {
  constructor(vars, name, stacksBasePath) {
    ...
    this.runtime = new Runtime(_Session.isLocal(), this.vars);
  }
  /**
   * Running locally or in lambda as part of integ.
   */
  static isLocal() {
    return process.env[CDK_ATMOSPHERE_INTEG_ENV] !== "true";
  }
  /**
  ...
  static async create(sessionName) {
    ...
    if (!_Session.isLocal() && stacksBucket && stacksKey) {
  ...
  async deployStack(opts) {
    ...
    const templatePath = _Session.isLocal() ? opts.templatePath : path.basename(opts.templatePath);
