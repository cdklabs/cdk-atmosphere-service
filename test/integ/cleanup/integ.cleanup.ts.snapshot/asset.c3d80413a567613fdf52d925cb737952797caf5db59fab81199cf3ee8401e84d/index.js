...
async function handler4(_) {
  ...
  await Session.assert(async (session) => {
    ...
    const s32 = new import_client_s33.S3({ region });
    await s32.putObject({ Bucket: bucketName, Key: "one.txt", Body: "one" });
    await s32.putObject({ Bucket: bucketName, Key: "two.txt", Body: "two" });
    await s32.deleteObject({ Bucket: bucketName, Key: "two.txt" });
    await session.allocations.end({ id: body.id, outcome: "success" });
    await session.environments.cleaning(body.id, account, region);
