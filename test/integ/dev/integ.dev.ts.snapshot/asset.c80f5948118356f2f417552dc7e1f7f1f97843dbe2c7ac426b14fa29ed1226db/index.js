...
<<<<<<<< HEAD:test/integ/cleanup/integ.cleanup.ts.snapshot/asset.f7ab82c1229a28df18f8bf6fd60fab4d12e3234d4b111155937e9175c625bba9/index.js
...
>>>>>>>> main:test/integ/dev/integ.dev.ts.snapshot/asset.23624d86537a6add7eef5ee30047ed96f387c8692a3b2612da4c5a9335d1b3b4/index.js
  } catch (e) {
...
>>>>>>>> main:test/integ/dev/integ.dev.ts.snapshot/asset.c80f5948118356f2f417552dc7e1f7f1f97843dbe2c7ac426b14fa29ed1226db/index.js
  }
  async allocate(body) {
...
async function handler3(_) {
  return Session.assert(async (session) => {
    ...
    await session.waitFor(async () => (await session.fetchEnvironment(account, region)).Item === void 0, waitTimeSeconds);
    session.log(`Waiting ${waitTimeSeconds} seconds for cleanup task to stop`);
    await session.waitFor(async () => await session.fetchStoppedCleanupTask(body.id) !== void 0, waitTimeSeconds);
    const task = await session.fetchStoppedCleanupTask(body.id);
    assert2.strictEqual(task.containers[0].exitCode, 0);
  });
}
if (Session.isLocal()) {
  void handler3({});
