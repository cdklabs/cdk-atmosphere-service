...
var Assert = class _Assert {
   ...
   */
  static async run(name, assertion) {
    const session = await _Assert.create(name);
    await session.clear();
    try {
  ...
  async resolveTemplatePath(templatePath) {
    ...
    if (stacksBucket && stacksKey) {
      ...
      await _Assert.unzip(stacksBucket, stacksKey, to);
...
async function handler6(_) {
  return Assert.run("default", async () => {
    return;
  });
}
if (Assert.isLocal()) {
