...
async function handler3(_) {
  await Session.assert(async (session) => {
    ...
    await session.waitFor(async () => (await session.fetchAllocation(body.id)).Item.end?.S !== void 0, waitTime);
    ...
    await session.waitFor(async () => await session.fetchCleanupTimeoutSchedule(body.id) !== void 0, waitTime);
